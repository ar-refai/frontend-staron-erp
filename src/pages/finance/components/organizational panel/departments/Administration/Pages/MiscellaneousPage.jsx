...
import { AddNewMiscellaneous, ChangeMiscellaneousStatus, ShowAllMiscellaneous, ShowMiscellaneous, UpdateMiscellaneous } from 'apis/AdministrationApi/OrganiazationalPanel';
import styled from '@emotion/styled';
import TagIcon from '@mui/icons-material/Tag';
import AddMiscellaneousDialog from './MiscellaneousComponents/AddMiscellaneous';
import UpdateMiscellaneousDialog from './MiscellaneousComponents/UpdateMiscellaneous';
...
const MiscellaneousPage = () => {
  ...
  // Fetch the Miscellaneous accounts data from the API
  const fetchMiscellaneousData = async () => {
    try {
      const response = await ShowAllMiscellaneous();
      if (response.status === 200) {
        setArData(response.data);
      }
    } catch (error) {
      console.error("Failed to fetch Miscellaneouss data:", error);
    }
  };
  ...
  const handleEditClick = async (id) => {
    const response  = await ShowMiscellaneous(id);
  ...
  const Deactive = async (id) => {
    try {
      const response = await ChangeMiscellaneousStatus(id);
